...
use aidb_core::types::MemoryWithEmbedding;
...
pub fn py_cosine_similarity(a: Vec<f32>, b: Vec<f32>) -> f64 {
    aidb_core::consolidate::cosine_similarity(&a, &b)
}
...
pub fn py_extractive_summary(memories: Vec<Bound<'_, PyDict>>) -> PyResult<String> {
    let mems: Vec<MemoryWithEmbedding> = memories
        ...
        .collect::<PyResult<_>>()?;
    Ok(aidb_core::consolidate::extractive_summary(&mems))
}
...
pub fn py_find_clusters(
    ...
    let cluster_indices = aidb_core::consolidate::find_clusters(
        &mems,
        sim_threshold,
        time_window_days,
        min_cluster_size,
...
pub fn find_consolidation_candidates(
    ...
    let clusters = aidb_core::consolidate::find_consolidation_candidates(
        inner,
        sim_threshold,
        time_window_days,
        min_cluster_size,
...
pub fn py_consolidate(
    ...
    let inner = db.get_inner()?;
    let results = aidb_core::consolidate::consolidate(
        inner,
        sim_threshold,
        time_window_days,
        min_cluster_size,
