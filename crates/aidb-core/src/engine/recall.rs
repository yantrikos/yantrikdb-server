    ...
    pub fn recall(
        ...
        let mut scored: Vec<RecallResult> = Vec::new();
        {
            ...
            for (rid, distance) in &vec_results {
                ...
                let composite = scoring::composite_score_with_sentiment(sim_score, decay, recency, row.importance, row.valence, query_sentiment);
        ...
        // fallback because HNSW approximate search degrades with more vectors.
        {
            ...
            if !important_rids.is_empty() {
                ...
                for rid in &important_rids {
                    ...
                    let age = ts - row.created_at;
                    let recency = scoring::recency_score(age);
                    let composite = scoring::composite_score_with_sentiment(
                        sim_score, decay, recency, row.importance, row.valence, query_sentiment,
                    );
                    let why = scoring::build_why(sim_score, recency, decay, row.valence);
                    let contributions =
                        scoring::standard_contributions(sim_score, decay, recency, row.importance);
                    let valence_multiplier = scoring::query_valence_boost(row.valence, query_sentiment);
...

            {
                if let Some(qt) = query_text {
                    ...
                    if !keywords.is_empty() {
                        ...
                        const KEYWORD_BOOST_BASE: f64 = 0.31;
                        {
                            ...
                            for result in &mut scored {
                                    ...
                                    && !result.why_retrieved.iter().any(|w| w == "keyword_match")
                                {
                                    ...
                                    let boost = KEYWORD_BOOST_BASE * (1.0 - sim).max(0.2);
                                    result.score += boost;
                        ...
                        if !new_fts_rids.is_empty() {
                            ...
                            for rid in &new_fts_rids {
                                ...
                                let kw_boost = KEYWORD_BOOST_BASE * (1.0 - sim_score).max(0.2);
                                ...
                                let contributions = scoring::standard_contributions(
                                    sim_score, decay, recency, row.importance,
                                );
                                let valence_multiplier = scoring::query_valence_boost(row.valence, query_sentiment);
            ...
            if !seed_entities.is_empty() && base_boost > 0.0 {
                ...
                if !filtered_rids.is_empty() {
                    ...
                    for rid in &filtered_rids {
                        ...
                        let prox = gi.graph_proximity(rid, &expanded_map);
                        let composite = scoring::graph_composite_score_with_sentiment(
                            sim_score, decay, recency, row.importance, row.valence, prox, query_sentiment,
                        );
                ...
                if !new_rids.is_empty() {
                    ...
                    for rid in &new_rids {
                        if let (Some(row), Some(emb_blob)) = (cache.get(rid.as_str()), emb_map.get(rid.as_str())) {
                            ...
                            let recency = scoring::recency_score(age);
                            let prox = gi.graph_proximity(rid, &expanded_map);
                            let composite = scoring::graph_composite_score_with_sentiment(sim_score, decay, recency, row.importance, row.valence, prox, query_sentiment);
                            let mut why = scoring::build_why(sim_score, recency, decay, row.valence);
